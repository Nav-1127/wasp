...
/* DemoSection — v13 (2026-04-30)
 * 7 scenes: WASP enters → priority selection → replies → sensitivity routing
 *           → comment to DM → sting triggers → outcome
 * Volume-problem opener removed (Problem section above covers it).
 * Scenes 2 (priority) and 4 (sensitivity) are illustrative — will be replaced
 * with real product walkthrough after dashboard redesign ships.
 */
...
/* ─────────────────────────────────────────────────
   SCENE TIMING
───────────────────────────────────────────────── */
const SCENE_DUR = [4000, 6000, 8000, 6000, 8000, 8000, 5000]; // 45 000 ms
...
const CAPTIONS = [
  "WASP reads every comment and DM.",
  "High-intent ones get answered first.",
  "Replies in your voice. Instantly.",
  "Sensitive comments handled carefully.",
...
const SENSITIVE_A: CommentItem = { id: 97, user: "jake.m",  bg: "#F0C3C3", text: "quality seems worse than the last drop honestly", intent: "high" };
const SENSITIVE_B: CommentItem = { id: 98, user: "sarah.k", bg: "#EDC3C3", text: "why is shipping taking so long??",                 intent: "high" };
const DISCOUNT:    CommentItem = { id: 99, user: "discount_dani", bg: "#F0C3C3", text: "can I get a discount code?",                 intent: "high" };

/* ─────────────────────────────────────────────────
   SCENE TIMER — with jump support
───────────────────────────────────────────────── */
...
function IGDesktopShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="wasp-demo-window" style={{ width: "100%", background: "#fff", borderRadius: 16, overflow: "hidden", boxShadow: "0 2px 6px rgba(0,0,0,0.04),0 10px 32px rgba(0,0,0,0.11),0 28px 72px rgba(0,0,0,0.07)", display: "flex", height: 560 }}>
      <div className="wasp-demo-photo" style={{ width: "42%", flexShrink: 0, position: "relative", background: "#DDD5C8", borderRight: "1px solid #dbdbdb" }}>
        <Image src="https://images.unsplash.com/photo-1556821840-3a63f95609a7?w=636&h=800&fit=crop&q=80&auto=format" alt="northform drop 06 hoodie" fill style={{ objectFit: "cover" }} unoptimized />
      </div>
...
/* ─────────────────────────────────────────────────
   COMMENT ROW VARIANTS
───────────────────────────────────────────────── */
function CRow({ c, glow = false, replyVisible = false, replyText = "" }: {
  c: CommentItem; glow?: boolean; replyVisible?: boolean; replyText?: string;
}) {
  return (
    <div style={{ background: glow ? "rgba(91,43,140,0.055)" : "transparent", borderLeft: glow ? "2px solid rgba(91,43,140,0.22)" : "2px solid transparent", transition: "background 0.55s ease, border-color 0.55s ease" }}>
      <div style={{ display: "flex", gap: 8, padding: "6px 12px 2px", alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: igFont, fontSize: 12.5, color: "#000", lineHeight: 1.35 }}><b>{c.user}</b> {c.text}</div>
          <div style={{ display: "flex", gap: 10, fontFamily: igFont, fontSize: 11, color: "#8e8e8e", marginTop: 2 }}><span>2h</span><span style={{ cursor: "pointer" }}>Reply</span></div>
        </div>
      </div>
      {replyText && (
        <div style={{ padding: "2px 12px 6px 44px", maxHeight: replyVisible ? 70 : 0, overflow: "hidden", opacity: replyVisible ? 1 : 0, transform: replyVisible ? "translateY(0)" : "translateY(4px)", transition: "max-height 0.4s ease, opacity 0.4s ease, transform 0.4s ease" }}>
          <div style={{ display: "flex", gap: 7, alignItems: "flex-start" }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: igFont, fontSize: 12.5, color: "#000", lineHeight: 1.35 }}><b>northform</b><VBadge /> {replyText}</div>
...
function CRowPriority({ c, glow = false, badge }: { c: CommentItem; glow?: boolean; badge?: number }) {
  return (
    <div style={{ background: glow ? "rgba(91,43,140,0.055)" : "transparent", borderLeft: glow ? "2px solid rgba(91,43,140,0.22)" : "2px solid transparent", transition: "background 0.55s ease, border-color 0.55s ease" }}>
      <div style={{ display: "flex", gap: 8, padding: "6px 12px 2px", alignItems: "flex-start" }}>
        <Avatar initial={c.user[0].toUpperCase()} bg={c.bg} size={24} />
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: igFont, fontSize: 12.5, color: "#000", lineHeight: 1.35 }}><b>{c.user}</b> {c.text}</div>
          <div style={{ display: "flex", gap: 10, fontFamily: igFont, fontSize: 11, color: "#8e8e8e", marginTop: 2 }}><span>2h</span><span>Reply</span></div>
        </div>
        {badge !== undefined && (
          <div style={{ opacity: 1, transform: "scale(1)", transition: "opacity 0.3s ease, transform 0.3s ease", background: "#5B2B8C", borderRadius: 10, padding: "2px 7px", fontFamily: "Inter, sans-serif", fontSize: 10, fontWeight: 600, color: "#fff", flexShrink: 0, marginTop: 3 }}>
            P{badge}
          </div>
        )}
      </div>
...
/* ─────────────────────────────────────────────────
   SCENE 0 — WASP enters (shimmer sweep)
───────────────────────────────────────────────── */
function S0({ progress }: { progress: number }) {
      ...
      <div style={{ position: "relative", flexShrink: 0 }}>
        ...
        {!shimmered && (
          <div style={{ position: "absolute", inset: 0, pointerEvents: "none", overflow: "hidden" }}>
            <div style={{ position: "absolute", inset: 0, background: "linear-gradient(105deg,transparent 20%,rgba(255,255,255,0.6) 50%,transparent 80%)", animation: "waspDemoShimmer 1.15s cubic-bezier(0.4,0,0.2,1) 0.25s forwards" }} />
          </div>
        )}
      </div>
      <AddCommentBar />
...
/* ─────────────────────────────────────────────────
   SCENE 1 — Priority selection (glow + P badges)
───────────────────────────────────────────────── */
function S1({ progress }: { progress: number }) {
  const highPos = [1, 3, 5, 8, 9]; // indices into CORE that are high-intent
  const elapsed = progress * 6;
  const times   = [0.7, 1.5, 2.4, 3.4, 4.5];
  return (
    <>
      <div className="wasp-demo-no-sb wasp-demo-area" style={{ overflowY: "hidden", flexShrink: 0 }}>
        {CORE.map((c, i) => {
          const hIdx  = highPos.indexOf(i);
          const shown = hIdx >= 0 && elapsed >= times[hIdx];
          return (
            <CRowPriority
              key={c.id}
              c={c}
              glow={shown}
              badge={shown ? hIdx + 1 : undefined}
            />
          );
        })}
      </div>
...
/* ─────────────────────────────────────────────────
   SCENE 2 — Reply at scale (replies appear)
───────────────────────────────────────────────── */
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column", background: "#fff" }}>
      ...
      <div style={{ flex: 1, padding: "12px 12px 8px", display: "flex", flexDirection: "column", gap: 8, justifyContent: "flex-end" }}>
        <div style={{ display: "flex", justifyContent: "flex-start" }}>
          <div style={{ background: "#efefef", borderRadius: "18px 18px 18px 4px", padding: "8px 12px", fontFamily: igFont, fontSize: 13, color: "#000", maxWidth: "80%" }}>can I get a discount code?</div>
        </div>
        <div style={{ display: "flex", justifyContent: "flex-end", opacity: msgVisible ? 1 : 0, transform: msgVisible ? "translateY(0)" : "translateY(8px)", transition: "opacity 0.45s ease, transform 0.45s ease" }}>
          <div style={{ background: "#3797F0", borderRadius: "18px 18px 4px 18px", padding: "8px 12px", fontFamily: igFont, fontSize: 13, color: "#fff", maxWidth: "88%" }}>hey! here's a code just for you: TERRY15. valid through sunday 🤎</div>
        </div>
  ...
  return (
    <div className="wasp-demo-area" style={{ overflow: "hidden", flexShrink: 0, padding: "10px 12px", display: "flex", flexDirection: "column", gap: 7 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span style={{ fontFamily: "Inter, sans-serif", fontSize: 12, fontWeight: 600, color: "#0A0A0A" }}>Sting rules</span>
...
function SceneCaption({ scene, progress }: { scene: number; progress: number }) {
  ...
  return (
    <div style={{ textAlign: "center", marginBottom: 28, minHeight: 100 }}>
      ...
      <div key={scene} style={{ fontFamily: "var(--font-playfair), 'Playfair Display', Georgia, serif", fontWeight: 700, fontSize: isOutcome ? "clamp(1.5rem,2.2vw,2rem)" : "clamp(1.6rem,2.5vw,2.25rem)", lineHeight: 1.2, color: "#0A0A0A", letterSpacing: "-0.02em", opacity: op, transition: "opacity 0.08s", maxWidth: 600, margin: "0 auto", animation: "waspDemoFadeIn 0.42s ease forwards" }}>
        {CAPTIONS[scene]}
      </div>
    </div>
  );
...
───────────────────────────────────────────────── */
function ProgressBar({ scene, progress, onJump }: { scene: number; progress: number; onJump: (i: number) => void }) {
  return (
    <div style={{ display: "flex", gap: 6, width: "100%", marginTop: 0, marginBottom: 16 }}>
      {LABELS.map((label, i) => {
...
export default function DemoSectionLight() {
  ...
  return (
    <section id="how-it-works" style={{ width: "100%", borderTop: "1px solid #EBE5DC" }}>
      <style>{`
        @keyframes waspDemoFadeIn  { from { opacity: 0; } to { opacity: 1; } }
        @keyframes waspDemoShimmer { from { transform: translateX(-120%); } to { transform: translateX(220%); } }
        ...
        @media (max-width: 760px) {
          ...
          .wasp-demo-wrap { padding: 48px 20px 40px !important; }
        }
      `}</style>

      <div className="wasp-demo-wrap" style={{ maxWidth: 1060, margin: "0 auto", padding: "40px 40px 40px" }}>
        ...
        <div style={{ marginTop: 32, display: "flex", justifyContent: "center", opacity: scene === 6 && progress > 0.25 ? 1 : 0, transition: "opacity 0.5s ease", pointerEvents: scene === 6 ? "auto" : "none" }}>
          <Link href="/signup" style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: "Inter, sans-serif", fontSize: "0.9375rem", fontWeight: 500, color: "#fff", background: "#0A0A0A", padding: "13px 26px", borderRadius: 8, textDecoration: "none" }}>
            Start free
